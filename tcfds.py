...
def load_compressed(path, trust_remote_code=False):
    log(f"  Loading {path}...")
    # 🛡️ Sentinel: Enforce weights_only=True to prevent Pickle insecure deserialization vulnerabilities
    data = torch.load(path, map_location='cpu', weights_only=True)
    from transformers import AutoModelForCausalLM, AutoTokenizer, AutoConfig
    mn = data['model_name']

    # 🛡️ Sentinel: Avoid implicit arbitrary code execution via trust_remote_code
...
def main():
    ...
    # 🛡️ Sentinel: Require explicit opt-in for remote code execution
