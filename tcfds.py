...
def data_aware_svd(W, Cov, eps):
    ...
    Returns (U, S, Vh, k, (eigvecs, sqrt_ev)) — eigenvectors and root eigenvalues
    are returned so callers can compute the data-aware error without a second
    eigendecomposition, while avoiding O(n^3) materialization of Cov_sqrt."""
    m, n = W.shape
    ...
    # ⚡ Bolt Optimization: Avoid materializing the dense O(n^2) covariance square
    # root matrix which takes O(n^3) to compute.
    # W_weighted = W @ U_cov @ \Sigma_cov^{1/2}
    W_c = W.to(compute_device) if W.device != compute_device else W
    W_weighted = (W_c @ eigvecs_c) * sqrt_ev_c.unsqueeze(0)
    ...
    # Vh_orig = Vh @ \Sigma_cov^{-1/2} @ U_cov^T
    inv_sqrt_ev_v = inv_sqrt_ev_c.to(Vh.device)
    eigvecs_v = eigvecs_c.to(Vh.device)
    Vh_orig = (Vh * inv_sqrt_ev_v.unsqueeze(0)) @ eigvecs_v.t()
    del Vh
    free_mem()
...
class TCFDSLinear(nn.Module):
    ...
    def from_weight(cls, W_f32, bias_f32, eps, store_dtype, Cov=None):
        ...
        cov_components = None
        if Cov is not None:
            U, S, Vh, k, cov_components = data_aware_svd(W_f32, Cov, eps)
        else:
